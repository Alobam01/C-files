...
private val /** The `ARX_Symb_D` class provides time series analysis capabilities for ARX_D Symbolic
...
class ARX_Symb_D (x: MatrixD, y: MatrixD, hh: Int, n_exo: Int, fname: Array [String],
      ...
      extends ARX_D (x, y, hh, n_exo, fname, tRng, hparam, bakcast, tForms) = debugf ("ARX_Symb_D", true)                          // debug function
...
object ARX_Symb_D extends MakeMatrix4TS
        ...
        val (/** Create an `ARX_Symb_D` object by building an input matrix xy and then calling the
    ...
    def apply (xe: MatrixD, y: VectorD, hh: Int, fname_ : Array [String] = null,
               ...
               bakcast: Boolean = false): ARX_Symb_D =, n_fEndo) = (fEndo.length, fExo.length)
        val (n_fExo, xy)      = ARX_Symb.buildMatrix (xe, y, hparam, fEndo, fExo, bakcast)
        val tForms                = makeMatrix4Y (y, hh, bakcast)
        val yy = if fname_ == null then formNames (xe.dim2, hparam, n_fEndo, n_fExo) else fname_
        fname
    end apply 
...
//  val y  = yy                                                         // full
    ...
    for p <- 6 to 6; q <- 4 to 4; s <- 1 to 1 do                        // number of lags (endo, exo); trend
        ...
        val /** The `ARX_Symb_DTest4` main function tests the `ARX_Symb_D` class on real data:
 *  Forecasting COVID-19 using Train and Test (TnT).
 *  Test forecasts (h = 1 to hh steps ahead forecasts).
 *  > runMain scalation.modeling.forecasting.aRX_Symb_DTest4
 */
@main def aRX_Symb_DTest4 (): Unit = = ARX_Symb_D (xe, y, hh)                                // create model for time series data
