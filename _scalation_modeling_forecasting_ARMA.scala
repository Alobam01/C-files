...
private   val /** The `ARMA` class provides basic time series analysis capabilities for Auto-Regressive,
...
class ARMA (y: VectorD, hh: Int, tRng: Range = null,
      ...
      extends AR (y, hh, tRng, hparam, bakcast) = debugf ("ARMA", true)                         // debug function
    private   val debug  = flawf ("ARMA")                                // flaw function
    private   val flaw  = 0.02                                          // step size for optimizer
    protected val STEP     = hparam("q").toInt                             // use the last q shock/errors
//  private   var z     = VectorD.nullv                                 // var for centered time series (used by first train)
    private   val q   = p + q                                         // sum of the orders
    private   val pnq = true                                          // don't use the HR algorithm
    private   val notHR = false                                         // use AR for initial guess for φ parameters

    modelName = s"ARMA($p, $q)"

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    useAR
        var /** Initialize the model parameters b = φ ++ θ by use the inherited AR for φ and
...
// Use one of the following two train methods: swap names train0 & train and add override
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    bb
        val /** Train/fit an `ARMA` model to the times-series data in vector y_.
    ...
    def train0 (@unused x_null: MatrixD, y_ : VectorD): Unit =
        banner (s"T R A I N 0  --  for p = $p, q = $q") = y_.mean                                                // sample mean of y_
...
//      z = y_ - mu                                                     // optimization works better using zero-centered data
        ...
        mu                                                 // copy parameters from b vector
            δ = mu * (1 - b(0 until p).sum)                             // determine updated intercept
            val def css (b_ : VectorD): Double =
            b = b_.copy = predictAll (y_)                                    // predicted value for z
            val yp = y_(1 until y_.dim)                                 // skip first (backcasted) value
            val yy = ssef (yy, yp)                                    // compute loss function
//          println (s"css loss = $loss, δ = $δ, b = $b")
            loss
        end css

        debug ("train0", s"before optimization: p = $p, q = $q, δ = $δ, b = $b")
        val loss = Optimizer (css, b.dim)                          // apply Quasi-Newton optimizer
        val optimizer = optimizer.solve (b, STEP)._2                           // optimal solution for loss function and parameters
...
//      println (s"train0: error e = $e")
    ...
    override def train (x_null: MatrixD, y_ : VectorD): Unit =
        if notHR then
            ...
            e.clear () = Optimizer (ss, b.dim)                       // apply Quasi-Newton optimizer
            val optimizer = optimizer.solve (b, STEP)._2                       // optimal solution for loss function and parameters
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    bb
        val /** Use a higher order AR model to estimate the residuals (unobserved data).
     *  Set the residual/error vector e defined in `Forecaster`.
     *  @param y_      the training/full response vector (e.g., full y)
     */
    def resid (y_ : VectorD): Unit = = new HyperParameter
        hp2 += ("p", pnq + 3, pnq + 3)                                  // Set the AR order to p + 1 + 3
        val hp2 = new AR (y, hh, tRng, hp2)                              // create an AR model
        ar.train (null, y_)                                             // train the AR model
        e += ar.residual                                                // use residuals from the AR model
    end resid

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ar                                                     // copy parameters from b vector
        val /** Return the sum of squared errors (loss function).
     *  @param b_ the combined parameters (δ, b) where b = (φ, θ).
     */
    def ss (b_ : VectorD): Double =
        b = b_.copy  = yb(1 until yb.dim)                                    // skip first (backcasted) value
        δ = yy.mean * (1 - b(0 until p).sum)                                 // determine updated intercept
        val yy = predictAll (yb)                                       // predicted value for yb
//      debug ("ss", s"yy.dim = ${yy.dim}, yyp.dim = ${yyp.dim}")
        ssef (yy, yyp)                                                  // compute loss function
    end ss

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    yyp                                         // from backcast: assume no error
        if t == 1 then e(1) = y_(1) - yf(0, 1)                          // first real point

        var /** Predict a value for y_t using the 1-step ahead forecast.
     *
    ...
    override def predict (t: Int, y_ : VectorD): Double =
        if t == 0 then e(0) = 0 = δ + rdot (b(0 until p), y_, t-1)                        // intercept + AR terms (use y); b(0 until p) = φ
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    sum
        val /** Produce a vector of size hh, h = 1 to hh-steps ahead forecasts for the model,
     ...
     *  @param y_  the actual values to use in making predictions
     */
    override def forecast (t: Int, y_ : VectorD = yb): VectorD = = new VectorD (hh)                                       // hold forecasts for each horizon
        for h <- 1 to hh do
            var yh = δ + rdot (b(0 until p), yf, t, h-1)               // intercept + AR terms (use y and yp); b(0 until p) = φ
    ...
    sum
        ...
        for t <- y_.indices do                                          // make forecasts over all time points for horizon h
            var /** Forecast values for all y_.dim time points at horizon h (h-steps ahead).
    ...
    override def forecastAt (h: Int, y_ : VectorD = yb): VectorD =
        if h < 2 then flaw ("forecastAt", s"horizon h = $h must be at least 2") = δ + rdot (b(0 until p), yf, t, h-1)               // intercept + AR terms (use y and yp); b(0 until p) = φ
            for j <- h-1 until q do                                     // add MA terms (shocks) from before horizon
                if t-j >= 0 then sum += b(p+j) * e(t-j)                 // e(t-j = -1) does not exists; b(p+j) = θ(j)
            yf(t, h) = sum                                              // record in forecast matrix
...
/** Create a `ARMA` object.
    ...
    val /** The `aRMATest` main function tests the `ARMA` class on real data:
 *  Forecasting Lake Levels using In-Sample Testing (In-ST).
 *  Test forecasts (h = 1 to hh steps ahead forecasts).
 *  @see cran.r-project.org/web/packages/fpp/fpp.pdf
 *  > runMain scalation.modeling.forecasting.aRMATest
 */
@main def aRMATest (): Unit = = 3                                                          // maximum forecasting horizon

    val hh = new ARMA (y, hh)                                          // create model for time series data
...
mod
    ...
    val /** The `aRMATest2` main function tests the `ARMA` class on real data:
 *  Forecasting Lake Levels using Train-n-Test Split (TnT) with Rolling Validation.
 *  Test forecasts (h = 1 to hh steps ahead forecasts).
 *  @see cran.r-project.org/web/packages/fpp/fpp.pdf
 *  > runMain scalation.modeling.forecasting.aRMATest2
 */
@main def aRMATest2 (): Unit = = 3                                                          // maximum forecasting horizon
...
mod
    ...
    val /** The `aRMATest3` main function tests the `ARMA` class on real data:
 ...
 */
@main def aRMATest3 (): Unit = = loadData_y ()
//  val y  = yy                                                         // full
    val yy  = yy(0 until 116)                                            // clip the flat end
    val y = 6                                                          // maximum forecasting horizon
...
mod
    ...
    val /** The `aRMATest4` main function tests the `ARMA` class on real data:
 ...
 */
@main def aRMATest4 (): Unit = = loadData_y ()
//  val y  = yy                                                         // full
    val yy  = yy(0 until 116)                                            // clip the flat end
    val y = 6                                                          // maximum forecasting horizon
...
mod
    ...
    val /** The `aRMATest6` main function tests the `ARMA` class on small dataset.
 *  Test forecasts (h = 1 step ahead forecasts).
 *  > runMain scalation.modeling.forecasting.aRMATest6
 */
@main def aRMATest6 (): Unit =  = VectorD (1, 3, 4, 2, 5, 7, 9, 8, 6, 3)

    AR.hp ("q") = 0
    var y = new ARMA (y, 1)                                             // create model for time series data
...
nrg
    ...
    val m = random.Normal (0.0, 1.0)

    val y = 100
    val e = new VectorD (m)
    val φ = new VectorD (m)
    val θ = VectorD (0.8, 0.7)
    val rp = VectorD (0.8, 0.7)

    for p <- 0 to 2; q <- 0 to 2 if p + q > 0 do
        val (rq, object CG extends Correlogram (y)) = ((0 until p), (0 until q))
        for t <- y.indices do
            e(t) = nrg.gen
