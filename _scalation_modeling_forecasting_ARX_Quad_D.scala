...
private val /** The `ARX_Quad_D` class provides basic time series analysis capabilities for
...
class ARX_Quad_D (x: MatrixD, y: MatrixD, hh: Int, n_exo: Int, fname: Array [String] = null,
      ...
      extends ARX_D (x, y, hh, n_exo, fname, tRng, hparam, bakcast, tForms) = debugf ("ARX_Quad_D", true)                     // debug function
...
object ARX_Quad_D extends MakeMatrix4TS
        ...
        val (/** Create an `ARX_Quad_D` object by building an input matrix x and then calling the constructor.
    ...
    def apply (xe: MatrixD, y: VectorD, hh: Int, fname_ : Array [String] = null,
               ...
               bakcast: Boolean = false): ARX_Quad_D =, xy) = ARX_Quad.buildMatrix (xe, y, hparam, bakcast)
        val tForms    = makeMatrix4Y (y, hh, bakcast)
        val yy = if fname_ == null then ARX_Quad.formNames (xe.dim2, hparam) else fname_
        fname
    end apply
    ...
    def rescale (xe: MatrixD, y: VectorD, hh: Int, fname_ : Array [String] = null,
                 ...
                 fEndo: Array [Transform] = null, fExo: Array [Transform] = null,
                 bakcast: Boolean = false,
                 tForm: VectorD | MatrixD => Transform = x => rangeForm(x)): ARX_Quad_D =
...
//      val mod = ARX_Quad_D (xe, y, hh)                                // create model for time series data
        val /** The `aRX_Quad_DTest4` main function tests the `ARX_Quad_D` class on real data:
 *  Forecasting COVID-19 using Train-n-Test Split (TnT) with Rolling Validation.
 *  Test forecasts (h = 1 to hh steps ahead forecasts).
 *  > runMain scalation.modeling.forecasting.aRX_Quad_DTest4
 */
@main def aRX_Quad_DTest4 (): Unit = = ARX_Quad_D.rescale (xe, y, hh)                        // create model for time series data
