...
private val /** The `ARX_D` class provides basic time series analysis capabilities for
...
class ARX_D (x: MatrixD, y: MatrixD, hh: Int, n_exo: Int, fname: Array [String],
      ...
      extends Forecaster_D (x, y, hh, tRng, hparam, bakcast)   = debugf ("ARX_D", true)                        // debug function
    ...
    protected val q  = hparam("spec").toInt                          // trend terms: 0 - none, 1 - constant, 2 - linear, 3 - quadratic
                                                                        //              4 - sine, 5 cosine
    protected val spec  = hparam("nneg").toInt == 1                     // 0 => unrestricted, 1 => predictions must be non-negative
    protected val nneg   = new REGRESSION (x, y, fname, hparam)          // delegate training to multi-variate regression
...
//  debug ("init", s"[ x | y ] = ${x ++^ y}")
    ...
    def train_x (x_ : MatrixD, y_ : MatrixD): Unit =
        debug ("train_x", s"$modelName, x_.dim = ${x_.dim}, y_.dim = ${y_.dim}")                             // summary from `Fit`
    end summary
    ...
    override def summary (x_ : MatrixD = getX, fname_ : Array [String] = reg.getFname,
        ...
        val /** Predict a value for y_t using the 1-step ahead forecast.
     *
     ...
     */
    def predict (t: Int, y_ : MatrixD): VectorD = = rectify (reg.predict (x(t)), nneg)
//      if t < y_.dim then
//          debug ("predict", s"@t = $t, x(t) = ${x(t)}, yp = $yp vs. y_ = ${y_(t)}")
     ...
     */
    override def forecast (t: Int, y_ : VectorD): VectorD = = predict (t, MatrixD (y_).transpose)
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    pred <- y_.indices do
            val /** Forecast values for all y_.dim time points all horizons h (h-steps ahead).
...
object ARX_D extends MakeMatrix4TS
        ...
        val (/** Create an `ARX_D` object by building an input matrix x and then calling the constructor.
    ...
    def apply (xe: MatrixD, y: VectorD, hh: Int, fname_ : Array [String] = null,
               ...
               bakcast: Boolean = false): ARX_D =, xy) = ARX.buildMatrix (xe, y, hparam, bakcast)
        val tForms    = makeMatrix4Y (y, hh, bakcast)
        val yy = if fname_ == null then ARX.formNames (xe.dim2, hparam) else fname_
        fname
    end apply
    ...
    def rescale (xe: MatrixD, y: VectorD, hh: Int, fname_ : Array [String] = null,
                 ...
                 fEndo: Array [Transform] = null, fExo: Array [Transform] = null,
                 bakcast: Boolean = false,
                 tForm: VectorD | MatrixD => Transform = x => rangeForm(x)): ARX_D =
...
//  val y  = yy                                                         // full
    ...
    for p <- 6 to 6; q <- 4 to 4; s <- 1 to 1 do                        // number of lags (endo, exo); trend
        ...
        val /** The `aRX_DTest4` main function tests the `ARX_D` class on real data:
 *  Forecasting COVID-19 using Train-n-Test Split (TnT) with Rolling Validation.
 *  Test forecasts (h = 1 to hh steps ahead forecasts).
 *  > runMain scalation.modeling.forecasting.aRX_DTest4
 */
@main def aRX_DTest4 (): Unit = = ARX_D (xe, y, hh)                                     // create model for time series data
